...
document.addEventListener("click", (e) => {
  const analyticsSidebar = document.querySelector("#analytics-sidebar");
  const analyticsSidebarToggler = document.querySelector(
    "#analytics-sidebar-toggler"
  );
  ...
  if (
    analyticsSidebar.contains(e.target) ||
    e.target === analyticsButton ||
    e.target === analyticsSidebarToggler
  ) {
    return;
  }
...
async function fetchOrders(url) {
  ...
  for (const order of orders) {
    ...
    if (hasPiniataDiscount(order)) {
      piniatasCounter++;
      totalPiniataDiscount += order.joker_discount;
    }
  }

  // Top 5 restaurant sorted by number of orders
  const favoriteRestaurants = findFavoriteRestaurants(restaurants, orders)
  ...
  const stats = {
    ...
    total_piniatas: piniatasCounter,
    total_piniatas_discount: totalPiniataDiscount,
...
function findFavoriteRestaurants(restaurants, orders) {
  ...
  for (const restaurant of restaurants) {
    ...
    if (
      !restaurantsWithCounter.find(
        (restaurant) => restaurant.name === restaurantName
      )
    ) {
      const timesOrdered = restaurants.filter(
        (restaurant) => restaurant[0] === restaurantName
      ).length;
      const restaurantExpenses = getExpensesPerRestaurant(
        orders,
        restaurantName
      );
...
function fillWithStats(sidebarContent, stats) {
  ...
  const html = `
        ...
        <hr>
        <div class="analytics-container">
            <div class="counter-box"> <h2> ${total_expenses.toFixed(
              0
            )} € </h2> </div>
        ...
        <hr>
        <div class="analytics-container">
            <div class="counter-box"> <h2> ${total_piniatas} </h2> </div>
            <h2> Συνολικές πινιάτες </h2>
        </div>
        <hr>
        <div class="analytics-container">
            <div class="counter-box"> <h2> -${total_piniatas_discount} € </h2> </div>
